...
export default {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option.setName("option").setDescription("사용할 기능").setRequired(false)
    ),
  ...
  async execute(interaction: any) {
    ...
    const embed = {
      ...
      author: {
        name: "Chat Log",
        icon_url:
          "https://cdn.discordapp.com/avatars/1011894019388883005/8dbf7d6a5839d45881bb1bb3998d68d1.webp",
      },
      title: "",
      fields: [] as any[],
    ...
    if (
      ...
      interaction.options._hoistedOptions[0].value == "list"
    ) {
      ...
      for (let i = 0; i < log[interaction.channelId].length; i++) {
        embed.fields.push({
          name: log[interaction.channelId][i].author,
          value: log[interaction.channelId][i].content,
        });
      }
